...
public class MockPipeline extends Pipeline {
    ...
    public MockPipeline(MockClient mockClient, MockStorage mockStorage) {
	...
	public Response<String> ping() {
		final Response<String> response = getResponse(BuilderFactory.STRING);
	...
	public Response<byte[]> echo(final byte[] string) {
		final Response<byte[]> response = getResponse(BuilderFactory.BYTE_ARRAY);
		response.set(string);
		return response;
	}
	...
	public Response<Boolean> exists(final String key) {
		final Response<Boolean> response = getResponse(BuilderFactory.BOOLEAN);
	...
	public Response<List<String>> mget(final String... keys) {
		final Response<List<String>> response = getResponse(BuilderFactory.STRING_LIST);
	...
	public Response<List<byte[]>> mget(final byte[]... keys) {
		final Response<List<byte[]>> response = getResponse(BuilderFactory.BYTE_ARRAY_LIST);
	...
	public Response<Map<String, String>> hgetAll(final String key) {
		final Response<Map<String, String>> response = getResponse(BuilderFactory.STRING_MAP);
	...
	public Response<Map<byte[], byte[]>> hgetAll(final byte[] key) {
		final Response<Map<byte[], byte[]>> response = getResponse(BuilderFactory.BYTE_ARRAY_MAP);
		final Map<DataContainer, DataContainer> result = mockStorage.hgetAll(DataContainer.from(key));
	...
	public Response<Set<String>> hkeys(final String key) {
		final Response<Set<String>> response = getResponse(BuilderFactory.STRING_SET);
	...
	public Response<Set<byte[]>> hkeys(final byte[] key) {
		final Response<Set<byte[]>> response = getResponse(BuilderFactory.BYTE_ARRAY_ZSET);
		final Set<DataContainer> result = mockStorage.hkeys(DataContainer.from(key));
		response.set(result == null ? new ArrayList<byte[]>(0) : DataContainer.toBytes(result));
		return response;
	}
    ...
    public Response<Long> zadd(String key, double score, String member) {
        final Response<Long> response = getResponse(BuilderFactory.LONG);
        response.set(mockStorage.zadd(DataContainer.from(key), score, member));
        return response;
    }
    ...
    public Response<Double> zscore(String key, String member) {
        Double result = mockStorage.zscore(DataContainer.from(key), DataContainer.from(member));
        if(result == null) {
            result = 0.0;
        }

        final Response<Double> response = getResponse(BuilderFactory.DOUBLE);
        response.set(String.valueOf(result).getBytes());
        return response;
    }
	...
	public Response<Set<String>> zrange(final String key, final long start, final long end){
		final Response<Set<String>> response = getResponse(BuilderFactory.STRING_ZSET);
		response.set(mockStorage.zrange(DataContainer.from(key), start, end));
		return response;
	}

    @Override
    public Response<Long> zrank(String key, String member) {
        return zrank(DataContainer.from(key), DataContainer.from(member));
    }
    ...
    public Response<Long> zrevrank(String key, String member) {
        return zrevrank(DataContainer.from(key), DataContainer.from(member));
    }
    ...
    public Response<Set<byte[]>> zrevrangeByScore(byte[] key, double max, double min, int offset, int count) {
        final Response<Set<byte[]>> response = getResponse(BuilderFactory.BYTE_ARRAY_ZSET);
        response.set(mockStorage.zrevrangeByScore(DataContainer.from(key), max, min, offset, count));
        return response;
    }

    @Override
    public Response<Set<Tuple>> zrevrangeWithScores(String key, long start, long end) {
        final Response<Set<Tuple>> response = getResponse(BuilderFactory.TUPLE_ZSET);
        response.set(mockStorage.zrevrangeWithScores(DataContainer.from(key), start, end));
        return response;
    }
